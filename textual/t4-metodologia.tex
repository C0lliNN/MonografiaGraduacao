...
Para realização do \english{design} deste projeto, é utilizado a \english{UML}. Especificamente, são criados diagramas de classes, diagramas de sequência, diagramas de casos de uso. Além disso, para modelagem da arquitetura do sistema, o diagrama de contexto de sistema do modelo C4 é empregado \cite{c4Model}. 
...
\subsubsection{Requisitos não funcionais}
Por outro lado, para validação dos requisitos não funcionais, testes de carga são empregados. O ambiente de execução desses testes será o mesmo de produção. No entanto, apenas duas instâncias são utilizadas. Através da ferramenta Gatling, a bateria de testes consistirá de 1000 operações sendo executadas a cada minuto por um período de 1 hora. Além disso, a cada 15 minutos haverá um pico de 10000 operações em um minuto. Serão avaliados o tempo de resposta, a utilização da CPU, o consumo de memória e a quantidade erros. Cada operação possui as seguintes etapas: registrar um novo veículo, cadastrar um cliente, listar veículos e realizar uma reserva.

Após a execução dos testes de carga, os resultados obtidos serão analizados e comparados com os requisitios não funcionais. Dessa forma, um relatório será produzido com uma análise gráfica e númerica.
